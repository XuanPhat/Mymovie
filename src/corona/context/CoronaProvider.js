...
class CoronaProvider extends React.Component {
	...
	async componentDidMount() {
		...
		if (!helper.isEmptyObject(data)) {
			...
			await this.setStateSync({ ...this.state, virus: {}, error: { cod: 404, mess: `Not found data` } });
		}
		await this.setStateSync({ ...this.state, loading: false });
	}

	render() {
		return <CoronaContext.Provider value={this.state}>{this.props.children}</CoronaContext.Provider>;
	}
}
